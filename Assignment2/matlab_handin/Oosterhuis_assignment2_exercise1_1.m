function [x] = Oosterhuis_assignment2_exercise1_1(func, start1, start2, max_number_iterations, desired_precision)
%T.J. Oosterhuis
%Oosterhuis_assignment2_exercise1_1 takes a function on R, two values for x start1 and start2 and a
...
%Output of function is a vector x with the x(i) element being the x_i value
%of the secant method. Length of this vector will be equal to
%the number of iterations that is used plus two.
%
...
%measure for desired precision is max absolut difference between x_n_plus_1 and x_n, default is 0.0001
if exist('desired_precision')
else desired_precision = 0.0001;
end

%define vector for x of length max_number_iterations + 2 (for start values),
% with first two values start1 and start2 given from userinput
